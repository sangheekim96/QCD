...
#' @examples
#' set.seed(1)
#' data = generate.data(n = 150, p = 150,
#' signal = 1, tau = 0.5, autocorrelation = 0.5)
#'
generate.data <- function(n, p, signal = 1, tau = 0.5, autocorrelation = 0.5) {
  ...
  Sigma <- autocorrelation^abs(outer(1:p, 1:p, '-'))
  X <- rmvnorm(n, mean = rep(0, p), sigma = Sigma)
  epsilon <- rnorm(n)
  Y <- rowSums(X[,indx]) * signal + 0.7 * pnorm(X[,1]) * epsilon
